...
const objectData = data
   // It converts the variable "data" into an array by identifying blank spaces.
  .split(" ")
  // Reduce method applied on data variable once it was converted into an array. Its utility is to iterate an array and to counter elements.
  .reduce( 
    // Accumulator parameter which its objective is to accumulate during the iteration. Current parameter indicates the current element of the index.
    (accumulator, current) => (
    // accumulator[current] = accumulator[current] + 1 access to the actual value in the "accumulator" object. If it doesn't exist, it will initialize with the default value 1.
    // accumulator[current] + 1 increments the current count by 1.
    // accumulator[current] + 1 || 1 if the element wasn't in the object "accumulator", it will initialize the count in the value 1.
    (accumulator[current] = accumulator[current] + 1 || 1), accumulator
    ),
    // It indicates the result variable is object data type.
    {}
  );

let dataString = "";
// This iteration access to the keys and values of the variable "objectData".
// After that, it concatenates and save the result into a variable the key and value in 1 string.
for (const [key, value] of Object.entries(objectData)) {
    dataString = dataString.concat(`${key}${value}`);
}
